...
export const StoreProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  useEffect(() => {
    refreshData();
    const sub = supabase.channel('sync_tostadora').on('postgres_changes', { event: '*', schema: 'public' }, () => refreshData()).subscribe();
  ...
  const importData = async (jsonData: string) => {
    ...
    try {
      ...
      // Importación secuencial por lotes para no saturar la conexión
      if (data.products) {
      ...
      if (data.purchases) {
        for (const pur of data.purchases) {
          await supabase.from('purchases').upsert({
            id: pur.id, date: pur.date, product_id: pur.productId, product_name: pur.productName,
            quantity: pur.quantity, unit_cost: pur.unitCost, total_cost: pur.totalCost
          });
        }
      }
      ...
      await refreshData();
      alert("¡Restauración Completa! Todos los módulos están sincronizados.");
    } catch (e) {
      alert("Error al importar. Revisa el archivo.");
    } finally {
      setIsLoading(false);
    }
  };
