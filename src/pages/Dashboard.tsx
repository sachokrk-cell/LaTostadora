...
const Modal = ({ title, children, onClose, maxWidth = 'max-w-2xl' }: ModalProps) => (
  <div className="fixed inset-0 bg-[#1C1C1C]/80 z-[100] flex items-end sm:items-center justify-center p-0 sm:p-4 backdrop-blur-sm animate-fade-in text-[#1C1C1C]">
    <div className={`bg-white rounded-t-[2.5rem] sm:rounded-[2rem] shadow-2xl w-full ${maxWidth} max-h-[95vh] sm:max-h-[90vh] flex flex-col border-t-4 sm:border-4 border-[#6B7A3A]`}>
      <div className="p-6 border-b border-gray-100 flex justify-between items-center bg-white rounded-t-[2.5rem] sm:rounded-t-[1.8rem] flex-shrink-0">
    ...
    <div className="p-4 sm:p-8 space-y-6 sm:space-y-10 pb-24 bg-white min-h-screen text-[#1C1C1C]">
        ...
        <div className="grid grid-cols-2 sm:flex sm:flex-wrap gap-2 w-full lg:w-auto">
          {['MES', '3MESES', 'AÑO', 'TODO'].map((type) => (
            <button 
              ...
              className="px-4 py-3 bg-[#1C1C1C] text-white rounded-2xl font-black text-[10px] sm:text-xs hover:bg-[#6B7A3A] transition-all active:scale-95 shadow-sm"
            >
              {type}
            </button>
          ))}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="relative z-10 flex flex-col h-full justify-between">
            <div className="flex justify-between items-start">
              ...
              <button 
                ...
                className="p-3 bg-[#6B7A3A] text-white rounded-2xl transition-all shadow-md active:scale-90"
              >
                <Settings size={24}/>
              </button>
            </div>
            ...
            <div className="mt-10 font-black">
              ...
              <div className="w-full h-3 sm:h-4 bg-gray-200 rounded-full overflow-hidden">
                <div 
                  className="h-full bg-[#6B7A3A] transition-all duration-1000 ease-out" 
        ...
        <div className="bg-[#1C1C1C] p-6 sm:p-8 rounded-[2.5rem] sm:rounded-[3rem] shadow-xl flex flex-col justify-center items-center text-center text-white border-b-8 border-[#6B7A3A]">
          ...
          <div className="mt-4 px-4 py-2 bg-[#6B7A3A]/20 rounded-2xl border border-[#6B7A3A]/30 text-[#6B7A3A]">
             <span className="font-black text-sm uppercase italic">Margen: {metrics.rev > 0 ? Math.round(((metrics.rev - metrics.cost)/metrics.rev)*100) : 0}%</span>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        <div className="bg-gray-50 p-6 rounded-[2rem] border-2 border-gray-100 lg:col-span-2 flex flex-col sm:flex-row items-center gap-6">
          ...
          <div className="w-full sm:w-1/2 space-y-4 font-black">
             ...
             <div className="flex items-center justify-between border-b border-gray-200 pb-2 text-[#6B7A3A]">
        ...
        <div className="bg-white p-6 rounded-[2rem] shadow-sm border-2 border-gray-100 flex flex-col justify-center items-center">
            <ShoppingBag className="text-[#5A3E2B] mb-2" size={32} />
            <p className="text-[10px] font-black uppercase text-[#6B7A3A] tracking-widest text-center">Ventas</p>
            <h4 className="text-3xl font-black">{metrics.unitsSold.toLocaleString()}</h4>
        </div>

        <div className="bg-red-50 p-6 rounded-[2rem] border-2 border-red-100 flex flex-col justify-center items-center text-red-600">
            <Utensils className="mb-2" size={32} />
            <p className="text-[10px] font-black uppercase tracking-widest text-center">Stock Crítico</p>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 sm:gap-8">
        <div className="bg-[#1C1C1C] p-6 sm:p-8 rounded-[2.5rem] sm:rounded-[3rem] shadow-xl text-white border-l-8 border-[#6B7A3A]">
          ...
          <div className="space-y-3">
            {topClients.map(([name, total], idx) => (
              <div key={idx} className="flex items-center justify-between bg-white/10 p-4 rounded-2xl border border-white/5 hover:bg-white/20 transition-all">
        ...
        <div className="bg-gray-50 p-6 sm:p-8 rounded-[2.5rem] sm:rounded-[3rem] border-2 border-gray-100 shadow-sm">
          <div className="flex items-center gap-3 mb-6 sm:mb-8">
            ...
            {topVarieties.slice(0, 5).map((v, idx) => (
              <div key={idx} className="flex items-center justify-between border-b border-gray-200 pb-2">
                <div className="flex items-center gap-3 font-bold text-[#1C1C1C]">
                  <span className="w-7 h-7 rounded-full bg-[#6B7A3A] text-white flex items-center justify-center font-black text-[10px]">{idx + 1}</span>
                  {v.name}
                </div>
            ...
            <AreaChart data={yearlyComparisonData}>
              <defs>
                <linearGradient id="colorAct" x1="0" y1="0" x2="0" y2="1">
                  <stop offset="5%" stopColor="#6B7A3A" stopOpacity={0.2}/>
              ...
              <YAxis hide />
              <Tooltip contentStyle={{borderRadius: '1.5rem', border: 'none', backgroundColor: 'white', boxShadow: '0 20px 25px -5px rgba(0,0,0,0.1)'}} />
